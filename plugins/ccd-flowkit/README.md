...
## 依存パッケージ脆弱性スキャン（必須の外部ツール: osv-scanner）
BLD_GATE で `scripts/security_scan.py` が自動実行され、プロジェクト配下の lockfile
（requirements.txt / package-lock.json / go.mod / Cargo.lock / pom.xml など 20+ 種）を
[osv-scanner](https://google.github.io/osv-scanner/) でスキャンし、
`evidence/security/osv-scan.json` に結果を保存します。high 深刻度が 1 件でもあれば
fail-closed で停止し、reviewer-security 相当が判断します。
...
# その他: https://github.com/google/osv-scanner/releases から SLSA3 署名済みバイナリ
```

未インストール時は `security_scan.py` が exit 2 で hard-fail し、ゲートが「未検査 green」を
返さないよう機械的に保護します（`osv-scanner` は OSV.dev = GHSA + PyPA + RustSec + Go
+ npm + distros + OpenSSF Malicious Packages を統合するデータ源で、API は無料・レート
制限なし）。
