...
## 入力 / 規約
docs/design/結合試験計画.md と実装ソース ／ docs/conventions/test-conventions.md

## タスク
連携シナリオの各試験IDを実施し、実績・合否・不具合IDを記録。NGは構築担当へ修正依頼し再試験。
...
## 出力
docs/test/結合試験結果.md。完了したら合否件数と残不具合を報告する。

## state 進行（必須）
試験が完了したら、結果文書を書いただけで終わらず、委任プロンプトで渡された絶対パスの
`manage_flow_state.py advance T_INT <pass|ng>` を実行して state を進める
